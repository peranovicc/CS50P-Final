__all__ = ['generate_assignment', 'generate_tests']
...
def generate_tests(json_payload, module_name='default_module_name'):
    '''
    ...
    for group, tests in grouped_tests.items():
        function_string += _generate_test_function(group, tests, data['function_name']) + '\n'

    return function_string

def _generate_test_function(group, tests, fn_name):
    fn_string = f'def test_{group}():'
    for test in tests:
        tmp = _generate_assertion(test.get('input'), test.get('output'), fn_name)
        fn_string += f'\n    {tmp}'
    return fn_string + '\n'

def _generate_assertion(input, output, fn_name):
